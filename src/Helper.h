...
public:
	...
	{
		if (IsAutograderMode())
			flags &= ~SDL_RENDERER_PRESENTVSYNC; // VSync is disabled only for true autograder runs.
	...
	static void SDL_RenderPresent(SDL_Renderer* renderer)
	{
		...
		/* Present locally even during input replay; only the autograder skips window presentation. */
		if (!_autograder_mode)
